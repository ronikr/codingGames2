...
function cellClicked(elCell, i, j) {
    if (!gState.gameStarted)startTimer();
    gState.gameStarted = true;
    if (gBoard[i][j] === MINE) {
...
function startTimer() {
    gState.gTimeElapsed = 0;
    gInterval = setInterval(function () {
        var elTimer = document.querySelector(".timer");
        gState.gTimeElapsed++;
        elTimer.innerHTML = String(gState.gTimeElapsed);
    }, 1000);
};
...
function handleHighScore() {
    ...
    if (gState.gTimeElapsed <= gBestTime) {
        gBestTime = gState/gTimeElapsed;
